package xyz.iwolfking.woldsvaults.custom.items.fruits;
    ...
    protected void successEaten(Level level, ServerPlayer sPlayer) {
        if(ServerVaults.get(level).isPresent()) {
            ...
            for(VaultModifier<?> mod : hunter_modifier) {
                MessageFunctions.sendMessage(sPlayer, new TextComponent("Added ").append(mod.getChatDisplayNameComponent(1)));
